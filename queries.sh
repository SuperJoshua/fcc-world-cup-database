...
echo "$($PSQL "select distinct(name) from teams inner join games on teams.team_id = games.opponent_id or teams.team_id = games.winner_id where games.year = 2014 and games.round = 'Eighth-Final' order by name")"
